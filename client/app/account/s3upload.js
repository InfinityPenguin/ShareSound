(function() {
	window.S3Upload = (function() {
		...
		S3Upload.prototype.executeOnSignedUrl = function(file, callback) {
			...
			this_s3upload = this;
			xhr = new XMLHttpRequest();
			xhr.open('GET', this.s3_sign_put_url + '?s3_object_type=' + file.type + '&s3_object_name=' + file.name + '&user=' + this.user._id, true);
			...
			xhr.onreadystatechange = function(e) {
				...
				if (this.readyState === 4 && this.status === 200) {
					...
					if (this.status === 400) return this_s3upload.onError('Invalid track name.');
